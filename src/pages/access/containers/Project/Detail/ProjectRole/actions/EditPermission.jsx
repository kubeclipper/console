...
class Edit extends ModalAction {
  init() {
    this.store = rootStore.projectRoleStore;
  ...
  static policy = this.isAdminPage ? 'projects:edit' : 'projectroles:edit';
  ...
  async getRoleTemplate() {
    await this.store.fetchRoleTemplates({ limit: -1, project: this.project });
  }
  ...
  onSubmit = () => {
    ...
    return this.store.edit(
      { id: name, resourceVersion, project: this.project },
      formTemplate
    );
  };
}
